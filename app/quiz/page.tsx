...
import GradientRangeInput from "@/components/atoms/RangeInputField";
...
export default function QuizPage() {
    ...
    // console.log("currentQuestion", selectedRange);
    ...
    const handleResponse = async (value: number) => {
        ...
        const newResponses = { ...responses, [questionId]: +value.toFixed(1) };
    ...
    const handleNext = () => {
        // if(quizQuestions[currentQuestion].type === "range"){
        //     const questionId = quizQuestions[currentQuestion].id;
        //     const newResponses = { ...responses, [questionId]: +selectedRange.toFixed(1) };
        ...
        console.log("🔍 Current question ID:", currentQuestionId, hasAnswered);
                ...
                {!isComplete ? (
                    <div className="text-center">
                        <div className="bg-black/20 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-6 sm:p-8 border border-white/10 max-w-2xl mx-auto">
                            ...
                            {quizQuestions[currentQuestion].type === "multi-choice" ? (
                                <div className="space-y-3 sm:space-y-4">
                                    {quizQuestions?.[currentQuestion]?.options?.map((label, index) => {
                                        const value = (quizQuestions?.[currentQuestion]?.options?.length || 0) - index;
                                        const isSelected =
                                            responses[
                                                quizQuestions[currentQuestion].id
                                            ] === value;
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-full p-4 sm:p-4 rounded-xl border-2 transition-all duration-200 touch-manipulation ${
                                                    isSelected
                                ...
                                <GradientRangeInput 
                                    category={quizQuestions?.[currentQuestion]?.category?.toUpperCase() || "HOLISTIC"}
                                    onChange={(range)=> {
                                    // setSelectedRange(range)
                                    handleResponse(range)
                                }} resetValue={currentQuestion}/>
                            </div>
                            }
                            <div className="flex flex-col gap-2 mt-2">
                                <p className="text-left font-light text-sm ">
                                    This is the explanatory line foe the
                                    question that goes under the slider.
                                </p>
                                {/* <button
                                    onClick={handleNext}
                                    disabled={
                                    ...
                                    className="px-6 py-3 sm:px-11 w-fit mx-auto border border-[#4a598d] mt-8 rounded-lg transition-all text-sm sm:text-base"
                                >
                                    Submit
                                </button> */}
                            </div>
                              ...
                              <svg className="w-8 h-8 sm:w-10 sm:h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                        ...
                                        <p className="text-gray-300 text-base sm:text-lg mb-4 leading-relaxed">
                                            ...
                                            completed all{" "}
                                            <strong>{ quizQuestions?.length} questions</strong> of your
                                        ...
                                        <div className="bg-gradient-to-r from-cyan-400/10 to-purple-400/10 rounded-lg p-4 mb-6 sm:mb-8 border border-cyan-400/20">
                                            <p className="text-cyan-300 text-sm sm:text-base">
                                                ✨ Your unique HOLO attunement
                                                pattern is ready to be revealed
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <div className="text-center">
                                <div className="bg-black/20 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-6 sm:p-8 border border-white/10 max-w-2xl mx-auto">
                                    <div className="mb-6 sm:mb-8">
                                        ...
                                        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-3 sm:mb-4 leading-tight">
                                            Please describe the most powerful
                                            experience you have had with AI.
                                        </h2>
